...
public final class GrayTransform extends Application {
	...
	static Image transform(Image in, ColorTransformer colorTransformer1, ColorTransformer colorTransformer2) {
		...
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				out.getPixelWriter().setColor(x, y, colorTransformer1.apply(x, y, in.getPixelReader().getColor(x, y)));
				out.getPixelWriter().setColor(x, y, colorTransformer2.apply(x, y, in.getPixelReader().getColor(x, y)));
			}
		}
		return out;
	}
	...
	public void start(Stage stage) {
		...
		// ファイル書き出し
		try {
			ImageIO.write(resultImg, "png", new File("ex11.png"));
		} catch (IOException e) {
